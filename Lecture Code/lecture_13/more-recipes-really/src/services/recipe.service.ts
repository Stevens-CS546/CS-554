...
const getRecipeUrl = (id) => { return `${recipeRootUrl}/${id}` };
const getImageUrl = (id) => { return `${serverRoot}/photos/${id}.jpg` };
...
export class RecipeService {
    ...
    getRecipeList(): Promise<RecipeInterfaces.IShortRecipe[]> {
        return this.http.get(recipeRootUrl)
            .map(res => {
                return res.json() || [];
            })
            .toPromise();
    ...
    downloadImage(recipeId: number) {
        ...
        return fileTransfer.download(url, storageLocation).then((entry) => {
        ...
        return recipeIsCached.then((cached) => {
            ...
            return recipe.then((recipe) => {
                return this.downloadImage(recipe.id).then((imageStorageUrl) => {
                    ...
                    return File.writeFile(filePath, fileName, JSON.stringify(recipe), true).then(() => {
                        return recipe;
                    });
                });
            });
