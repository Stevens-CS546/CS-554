...
export const processRequest = function (uri, method = "GET", data, authenticated = false) {
    ...
    if (typeof data.query !== 'undefined') {
        const queryArray = [];
        Object.keys(data.query).forEach((key) => queryArray.push((`${key}=${encodeURIComponent(data.query[key])}`)));
    ...
    return fetch(url, request).then(function (response) { return processResponse(response); });
}
